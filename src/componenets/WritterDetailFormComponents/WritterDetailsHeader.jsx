...
const WritterDetailsHeader = () => {
  ...
  function saveWriterImage() {
    if (selectedFile) {
      if (preview != image) {
        dispatch(getWriterImage(selectedFile));
      } else {
        ...
        <div className="uploadWriterImageMain">
          ...
          <div>
            <div className="uploadBox">
              ...
              <img
                src={imgUrl ? image : selectedFile ? preview : Img}
                className="writerImge"
              />
              <input
                ref={photoRef}
                type="file"
          ...
          <div className="uploadWriterImageContent">
            <h1>Profile Photo</h1>
            {changesAlert ? (
              <Alert severity="error">You don't have any changes to save</Alert>
            ) : (
              <p>
            ...
            <div className="uploadBtns">
              {
                <button className="blue ">
                  <label
                    ...
                    htmlFor="attachImageWriter"
                  >
                    Update
                  </label>
                  <input
                    ref={photoRef}
                    id="attachImageWriter"
                    type="file"
                    accept="image/*"
                    name="file"
              ...
              <button
                style={{ cursor: "pointer" }}
                className="red"
                onClick={removeWriterImage}
              >
                Remove
              </button>
              <button
                className={`red ${imgLoading && " noClick"}`}
                onClick={saveWriterImage}
              >
                Save
              </button>
            </div>
